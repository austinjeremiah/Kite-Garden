...
export default function DashboardPage() {
  const { data, isError, error, isPending } = useAsyncPoll([], fetchDashboardData, 5_000);
    ...
    <div className="flex flex-col h-screen overflow-hidden">
      ...
      <div className="px-8 py-5 border-b border-white/25 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-3 text-xs font-mono font-bold text-white/40">
          {isError && (
            <span className="text-amber-400/90 max-w-md truncate" title={String(error)}>
              API offline — set NEXT_PUBLIC_API_URL (default http://localhost:4000)
            </span>
          )}
          {!isError && !isPending && (
            <>
          ...
          {isPending && <span className="text-white/30">Connecting…</span>}
        </div>
      </div>
